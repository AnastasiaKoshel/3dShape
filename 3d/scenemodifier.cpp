...
void SceneModifier::zoomin()
{
	float r = m_torus->radius();
	r = r + 1.f;
	m_torus->setRadius(r);
}
void SceneModifier::zoomout()
{
	float r = m_torus->radius();
	r = r - 1.f;
	m_torus->setRadius(r);
...
{
	QVector3D a= torusTransform->translation();
	a.setY(a.y()+1.f);
	torusTransform->setTranslation(a);

	//a = coneTransform->translation();
	//a.setY(a.y() + 1.f);
	//coneTransform->setTranslation(a);
}
void SceneModifier::down()
{
	QVector3D a = torusTransform->translation();
	a.setY(a.y() - 1.f);
	torusTransform->setTranslation(a);
...
void SceneModifier::rightrot()
{
	QVector3D a = torusTransform->rotation();
	a.setX(a.x() + 1.f);
	torusTransform->setRotation(a);

}*/

void SceneModifier::red()
{
	torusMaterial->setDiffuse(QColor(255, 0, 0, 127));
}
void SceneModifier::yellow()
{
	torusMaterial->setDiffuse(QColor("#ffff66"));
}
void SceneModifier::blue()
{
	torusMaterial->setDiffuse(QColor("#33ffcc"));
}
